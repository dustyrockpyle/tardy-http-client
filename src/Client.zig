...
pub const ConnectionPool = struct {
    ...
    pub fn release(pool: *ConnectionPool, allocator: Allocator, connection: *Connection) void {
        // Don't lock the pool while closing a connection; if two Frames try to close the socket at the same time,
        // this will deadlock the pool.
        ...
        var to_close: ?*Node = null;
        {
            ...
            if (pool.free_size == 0) {
                // Handle the case where the pool was resized to 0 during the release.
                to_close = node;
            } else {
        ...
        if (to_close) |popped| {
            // It's now safe to close the popped connection properly.
...
pub const Request = struct {
    ...
    fn redirect(req: *Request, uri: Uri) !void {
        ...
        const rt = req.connection.?.stream.rt;
        ...
        req.uri = valid_uri;
        req.connection = try req.client.connect(rt, new_host, uriPort(valid_uri, protocol), protocol);
...
pub fn connectTcp(client: *Client, rt: *tardy.Runtime, host: []const u8, port: u16, protocol: Connection.Protocol) ConnectTcpError!*Connection {
    ...
    const stream = Stream.tcpConnectToHost(rt, client.allocator, host, port) catch |err| switch (err) {
...
pub fn connectUnix(rt: *tardy.Runtime, client: *Client, path: []const u8) !*Connection {
    ...
    const stream = try Stream.connectUnixSocket(rt, path);
    errdefer stream.close();
...
pub fn connectTunnel(
    ...
    (tunnel: {
        const conn = try client.connectTcp(rt, proxy.host, proxy.port, proxy.protocol);
        errdefer {
            conn.closing = true;
        ...
        var req = client.open(rt, .CONNECT, .{
            .scheme = "http",
...
pub fn connect(
    ...
    } orelse return client.connectTcp(rt, host, port, protocol);

    // Prevent proxying through itself.
    if (std.ascii.eqlIgnoreCase(proxy.host, host) and
        proxy.port == port and proxy.protocol == protocol)
    {
        return client.connectTcp(rt, host, port, protocol);
    }

    if (proxy.supports_connect) tunnel: {
        return client.connectTunnel(rt, proxy, host, port) catch |err| switch (err) {
...
pub fn open(
    ...
    const conn = options.connection orelse
        try client.connect(rt, valid_uri.host.?.raw, uriPort(valid_uri, protocol), protocol);
...
fn fetchInternal(client: *Client, rt: *tardy.Runtime, options: FetchOptions) !http.Status {
    ...
    var req = try client.open(rt, method, uri, .{
